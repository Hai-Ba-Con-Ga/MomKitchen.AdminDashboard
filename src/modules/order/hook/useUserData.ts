...
const useUserData = () => {
  ...
  const createKitchenOwnerFunction = async (user: User) => {
    const response = await UserApi.createKitchen(user);
    return response.data;
  };
  const createKitchenOwner = useMutation(createKitchenOwnerFunction, {
    // You can specify onSuccess and onError callbacks here
  });
