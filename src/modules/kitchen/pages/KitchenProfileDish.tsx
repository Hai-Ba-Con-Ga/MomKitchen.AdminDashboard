...
const KitchenProfileDish = () => {
  ...
  const {
    deleteDish: { mutateAsync: deleteDishFunc },
  } = useDishData();
  const { setIdGetDish, kitchenDish, refreshKitchenDishData } =
    useKitchenData();
  ...
  return (
    <Box component="div">
      <Stack direction="row">
        <Swiper
          spaceBetween={50}
          slidesPerView={kitchenDish?.data?.length > 0 ? 3 : 1}
        ...
        </Swiper>
        {
          <Dialog
            ...
            <DishAddForm
              ...
              dish={editDish}
              onSuccessCallback={() => refreshKitchenDishData()}
              kitchenId={id}
            />
          </Dialog>
        }
